...
return new class extends Migration
{
    ...
    {
        return <<<'SQL'
        DROP VIEW IF EXISTS `tb_rens_usia`;
        SQL;
    }
    ...
    private function createView(): string
    {
        ...
        END) AS `range_usia`,
        COUNT(`dummy_table`.`usia`) AS `jumlah_perempuan`
            FROM
                (SELECT
                `db_sareng`.`tb_penduduk`.`id` AS `id`,
                `db_sareng`.`tb_penduduk`.`nik` AS `nik`,
                `db_sareng`.`tb_penduduk`.`nama` AS `nama`,
                `db_sareng`.`tb_penduduk`.`kelamin` AS `kelamin`,
                `db_sareng`.`tb_penduduk`.`tgl_lahir` AS `tgl_lahir`,
                TIMESTAMPDIFF(YEAR, `db_sareng`.`tb_penduduk`.`tgl_lahir`, CURDATE()) AS `usia`
                FROM
                    `db_sareng`.`tb_penduduk`) `dummy_table`
            WHERE
                (`dummy_table`.`kelamin` = 'perempuan')
            GROUP BY `range_usia`
            ORDER BY `range_usia`;
        SQL;
    }
